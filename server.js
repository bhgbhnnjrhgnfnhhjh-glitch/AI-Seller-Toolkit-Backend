...
// ==========================================================
// V18 SEO BRAND ISOLATION
// ==========================================================

function removeBrandFromProductName(productName, brand) {
    let product = cleanSEOKeyword(productName);
    const b = cleanSEOKeyword(brand);
    if (!product || !b) return product;

    const productTokens = normalizeWords(product);
    const brandTokens = normalizeWords(b);
    if (!brandTokens.length) return product;

    // Remove brand only when it appears as a contiguous prefix or suffix.
    const same = (a, b) => a.join(" ") === b.join(" ");
    if (productTokens.length >= brandTokens.length) {
        if (same(productTokens.slice(0, brandTokens.length), brandTokens)) {
            productTokens.splice(0, brandTokens.length);
        } else if (same(productTokens.slice(-brandTokens.length), brandTokens)) {
            productTokens.splice(-brandTokens.length);
        }
    }

    return cleanSEOKeyword(productTokens.join(" ")) || product;
}
...
function keywordContainsFullBrand(keyword, brand) {
    ...
    return b.length > 0 && b.every(token => k.has(token));
}
...
function buildInlineSEOKeywords(mainKeyword, productName, brand) {
    ...
    // Remove the exact product phrase and separately supplied brand.
    let remaining = [...sourceTokens];
    for (const token of productTokens) {
        const i = remaining.indexOf(token);
        if (i >= 0) remaining.splice(i, 1);
    }
    for (const token of normalizeWords(brand || '')) {
        const i = remaining.indexOf(token);
        if (i >= 0) remaining.splice(i, 1);
    }

    const connectorWords = new Set([
        'with','and','or','for','the','a','an','is','are','of','in','on','by','to','from','at'
    ]);
    remaining = remaining.filter(t => !connectorWords.has(t));
    remaining = remaining.filter(t => t !== 'color' && t !== 'colour');
    ...
    // Keep factual multi-word attributes together when the seller supplied
    // a natural phrase such as "floral print".
    const descriptors = [];
    for (let i = 0; i < remaining.length; i++) {
        const token = remaining[i];
        const next = remaining[i + 1] || '';
        if (token === 'floral' && next === 'print') {
            descriptors.push('floral print');
            i++;
        } else {
    ...
    // Main product first; never emit the raw sentence-like main keyword.
    pushKeyword(result, product);

    // A multi-word descriptor is kept intact. Do not split "floral print"
    // into the weaker and less natural "floral" keyword.
    for (const d of descriptors) {
        if (d.includes(' ')) {
            pushKeyword(result, `${d} ${product}`);
        } else {
            pushKeyword(result, `${d} ${product}`);
        }
    }

    // Pair factual attributes in source order.
    if (descriptors.length >= 2) {
        for (let i = 0; i < descriptors.length; i++) {
            for (let j = i + 1; j < descriptors.length; j++) {
                const first = descriptors[i];
                const second = descriptors[j];
                pushKeyword(result, `${first} ${second} ${product}`);
            }
        }
    }

    // Useful product-head variants for phrases such as "floral print kurti".
    const productHead = productTokens[productTokens.length - 1];
    const productPrefix = productTokens.slice(0, -1).join(' ');

    for (const d of descriptors) {
        pushKeyword(result, `${d} ${productHead}`);
    }

    if (descriptors.length >= 2) {
        for (let i = 0; i < descriptors.length; i++) {
            for (let j = i + 1; j < descriptors.length; j++) {
                pushKeyword(result, `${descriptors[i]} ${descriptors[j]} ${productHead}`);
                if (productPrefix) {
                    pushKeyword(result, `${productPrefix} ${descriptors[i]} ${descriptors[j]} ${productHead}`);
                }
            }
        }
    }
...
function buildFactualSEOKeywords(mainKeyword, productName, facts, body = {}) {
    ...
    const product = removeBrandFromProductName(productName, brand);
    ...
    // 4. V18: NEVER generate brand + product SEO combinations.
    // Brand is seller metadata, not an SEO modifier when supplied separately.
...
function keywordContainsProduct(keyword, productName) {
    const keywordTokens = new Set(normalizeWords(keyword));
...
function isValidSEOKeyword(keyword, productName, brand = "") {
    ...
    // Never expose a separately supplied brand inside SEO combinations.
    if (keywordContainsFullBrand(value, brand)) return false;

    const tokens = normalizeWords(value);
    if (!tokens.length) return false;

    // No connector-led/trailing phrases.
    const connectors = new Set([
        "with","and","or","for","the","a","an","is","are",
        "of","in","on","by","to","from","at","color","colour"
    ]);
    if (connectors.has(tokens[0]) || connectors.has(tokens[tokens.length - 1])) {
        return false;
    }

    // No duplicate token inside a keyword.
    if (new Set(tokens).size !== tokens.length) return false;

    // No standalone measurements at the beginning.
    if (/^(?:\d+(?:\.\d+)?|ml|l|g|kg|mg|cm|mm|m|inch|inches|ft|feet|piece|pieces)\b/i.test(value)) {
        return false;
    }

    // Product relevance: require the product head. For multi-word products,
    // keep at least the product head and reject modifier-only fragments.
    const productTokens = normalizeWords(productName);
    if (!productTokens.length) return false;
    const productHead = productTokens[productTokens.length - 1];
    if (!tokens.includes(productHead)) return false;

    // Reject a bare modifier + product-head phrase when the seller supplied
    // a stronger multi-word factual descriptor (e.g. "floral cotton kurti"
    // when the actual supplied phrase is "floral print").
    if (productTokens.length >= 2 && tokens.length === 2) {
        const modifier = tokens[0];
        const mainLower = cleanSEOKeyword(mainKeyword || '').toLowerCase();
        if (mainLower.includes(`${modifier} print`) || mainLower.includes(`${modifier} pattern`)) {
            return false;
        }
    }
...
function filterSEOKeywords(keywords, productName, mainKeyword = "", brand = "") {
    ...
    for (const raw of uniqueStrings(keywords)) {
        const keyword = cleanSEOKeyword(raw);
        if (!isValidSEOKeyword(keyword, productName, brand)) continue;
...
async function handleGenerateSEO(req, res) {
    try {
        ...
        const brand = cleanSEOKeyword(body.brand);
        const seoProductName = removeBrandFromProductName(productName, brand);

        const rawMainKeyword = cleanSEOKeyword(
            body.mainKeyword ||
            body.keyword ||
            body.productDetails ||
            seoProductName
        ...
        // Remove seller brand from the primary SEO phrase too.
        const mainKeyword = cleanSEOKeyword(
            normalizeWords(rawMainKeyword)
                .filter(token => !normalizeWords(brand).includes(token))
                .join(" ")
        ) || seoProductName;
...
5. If 3 or more explicit attributes are available, return only the
   useful natural combinations that are actually supported. Do not pad.
...
12. Never include the separately supplied brand in any keyword.
13. Never output a sentence or phrase containing connector words such as
    "with" or "and".
14. If the seller supplies "floral print", preserve that phrase; do not
    create the weaker "floral" variant.
...
`;
        ...
        // Final filter uses the brand-separated product name and the actual
        // separately supplied brand. Never force the raw primary sentence
        // into the output; only valid natural keywords survive.
