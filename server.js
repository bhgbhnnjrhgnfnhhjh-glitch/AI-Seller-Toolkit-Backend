...
function keywordContainsProduct(keyword, productName) {
    ...
    // At least two product-name tokens are required for multi-word products.
    if (matchedProductTokens < 2) return false;

    // For products with 3+ tokens, require at least 3 product tokens unless
    // the product itself is already fully represented.
    if (productTokens.length >= 3 && matchedProductTokens < 3) {
        const hasNonProductToken = keywordTokens.some(
            token => !productTokens.includes(token)
        );
        if (!hasNonProductToken) return false;
    }
...
function buildGuaranteedSEOKeywords(body, productName) {
    ...
    // Always start with the exact seller product name.
    pushKeyword(result, product);

    // Only use explicit structured seller fields. Raw free-text details are
    // deliberately excluded because they can create broken keywords such as
    // "inch size" or "suitable for gardening.".
    const fields = [
        "material", "fabric", "color", "size", "pattern", "design",
        "fit", "occasion", "quantity", "model", "connectivity",
        "compatibility", "capacity", "battery", "storage", "ram",
        "processor", "ingredients", "fragrance", "shade", "skinType",
        "hairType", "author", "language", "genre", "edition", "publisher",
        "isbn", "petType", "sport", "vehicleCompatibility", "sole",
        "soleType", "closure", "stone", "plating", "ageRange", "flavor",
        "usage", "weight", "dimensions", "packaging", "variant",
        "format", "pages", "includedItems", "warranty"
    ];

    const facts = [];
    for (const field of fields) {
        const value = cleanSEOKeyword(body[field]);
        if (value) pushKeyword(facts, value);
    }
    ...
    for (const fact of facts.slice(0, 8)) {
        pushKeyword(result, `${fact} ${product}`);
    }

    if (brand) {
        pushKeyword(result, `${brand} ${product}`);
        if (facts.length) pushKeyword(result, `${brand} ${facts[0]} ${product}`);
    }

    return uniqueStrings(result).filter(k =>
        isValidSEOKeyword(k, product, product, brand)
    ).slice(0, 20);
}
...
async function handleGenerateSEO(req, res) {
    try {
        ...
        // Product details are separate facts, never the primary keyword.
        // This also protects older frontends that accidentally sent
        // product + full details as mainKeyword.
        let mainKeyword = cleanSEOKeyword(
            body.mainKeyword ||
            body.keyword ||
            productName
        );
        const rawDetails = cleanSEOKeyword(body.productDetails);

        if (!mainKeyword || mainKeyword.toLowerCase() === rawDetails.toLowerCase()) {
            mainKeyword = productName;
        }

        if (rawDetails &&
            mainKeyword.toLowerCase().startsWith(productName.toLowerCase()) &&
            mainKeyword.length > productName.length) {
            mainKeyword = productName;
        }
...
`;
        ...
        // First try the strict quality filter.
        ...
        // IMPORTANT: never allow the endpoint to return success with an empty list.
        // If the strict filter rejects everything, use the deterministic seller-fact
        // fallback. This fallback never calls Gemini and never invents facts.
        if (!keywords.length) {
            keywords = buildGuaranteedSEOKeywords(body, productName);
        }

        // Always keep the primary product phrase first.
