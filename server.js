...
function buildNaturalSEOKeywords(body, productName) {
    ...
    // 1) Exact product phrase is ALWAYS first.
    add(product);

    // 2) Brand + exact product, only if seller supplied a brand.
    if (brand && !containsWholePhrase(product, brand)) {
        add(`${brand} ${product}`);
    }

    // Details may contain comma/semicolon/new-line separated seller facts.
    // We keep only facts that add information not already present in product.
    ...
    const extraFacts = [];
    for (const phrase of detailPhrases) {
        if (phrase.length < 2) continue;
        if (containsWholePhrase(product, phrase)) continue;
        if (extraFacts.some(x => x.toLowerCase() === phrase.toLowerCase())) continue;
        extraFacts.push(phrase);
    }

    // Also collect multi-word seller facts from structured fields, but never
    // duplicate facts already in product/details.
    const structured = [
        body.color, body.material, body.fabric, body.size,
        body.pattern, body.fit, body.occasion, body.quantity,
        body.capacity, body.weight, body.dimensions
    ].map(normalizeSEOKey).filter(Boolean);

    for (const fact of structured) {
        if (containsWholePhrase(product, fact)) continue;
        if (extraFacts.some(x => x.toLowerCase() === fact.toLowerCase())) continue;
        extraFacts.push(fact);
    }

    // Keep the product's natural noun phrase. For "8 Inch Plastic Plant Pot"
    // this becomes "plant pot", not "inch plastic plant pot".
    ...
    // Usually the final 1-3 words are the product noun. Prefer the last two
    // words when they form a meaningful noun phrase.
    let nounPhrase = nounWords.slice(-2).join(" ");
    if (!nounPhrase) nounPhrase = productWords.filter(w => !/^\d/.test(w)).slice(-2).join(" ");

    const facts = extraFacts
        .filter(f => phraseTokens(f).length <= 4)
        .slice(0, 6);

    // 3) Natural fact + product noun combinations.
    for (const fact of facts) {
        add(`${fact} ${nounPhrase}`);
    }

    // 4) Natural fact + product combinations, preserving the product phrase.
    for (const fact of facts) {
        add(`${fact} ${product}`);
        add(`${product} ${fact}`);
    }

    // 5) If there are 2+ extra facts, combine them without shuffling words.
    if (facts.length >= 2) {
    ...
    // 6) Clean and reject fragments. Every non-exact keyword must contain the
    // product noun phrase (e.g. "plant pot"), so "inch" and "plastic" can
    // NEVER appear alone.
    const final = [];
