...
function hasBrokenMeasurement(keyword) {
    ...
    const units = new Set([
        ...
        "inch", "inches", "piece", "pieces", "pcs", "pack", "packs"
    ]);
...
// ==========================================================
// DETERMINISTIC SEO FALLBACK
// Never depends on Gemini. Uses seller-provided facts only.
// ==========================================================
...
function buildGuaranteedSEOKeywords(body, productName) {
    ...
    // Canonical product phrase is always the first keyword.
    pushKeyword(result, product);
    ...
    // Collect only explicit seller attributes. Never split product details
    // into arbitrary single-word keywords such as "inch" or "plastic".
    const rawValues = [
        body.size, body.material, body.fabric, body.color, body.pattern,
        body.design, body.fit, body.occasion, body.quantity, body.model,
        body.connectivity, body.compatibility, body.capacity, body.battery,
        body.storage, body.ram, body.processor, body.ingredients,
        body.fragrance, body.shade, body.skinType, body.hairType,
        body.author, body.language, body.genre, body.edition, body.publisher,
        body.isbn, body.petType, body.sport, body.vehicleCompatibility,
        body.sole, body.soleType, body.closure, body.stone, body.plating,
        body.ageRange, body.flavor, body.usage, body.weight, body.dimensions,
        body.packaging, body.variant, body.format, body.pages,
        body.includedItems, body.warranty, body.features, body.productFeatures
    ].map(cleanSEOKeyword).filter(Boolean);

    // Also read comma/semicolon-separated detail phrases, but keep only
    // meaningful phrases (at least 2 characters) and never create a keyword
    // from a lone word that is already part of the product name.
    const detailPhrases = cleanSEOKeyword(body.productDetails)
        .split(/[,;|\n]+/)
        .map(cleanSEOKeyword)
        .filter(Boolean);

    const productTokens = new Set(seoTokens(product));
    const attributes = uniqueStrings([...rawValues, ...detailPhrases])
        .filter(value => {
            const tokens = seoTokens(value);
            if (!tokens.length) return false;
            if (tokens.length === 1 && productTokens.has(tokens[0])) return false;
            if (/^\d+(?:\.\d+)?$/.test(value)) return false;
            return true;
        });

    // Add natural attribute + product variants. Multi-word attributes are
    // preserved as supplied; no word-level permutations are generated.
    for (const attr of attributes.slice(0, 8)) {
        pushKeyword(result, `${attr} ${product}`);
    }

    // Build a clean noun phrase from the product name by removing explicit
    // size/material/color-like attributes when those values are known.
    const removable = [];
    for (const attr of attributes) {
        const toks = seoTokens(attr);
        if (toks.length <= 4) removable.push(toks);
    }

    let nounTokens = normalizeWords(product);
    for (const toks of removable) {
        if (!toks.length) continue;
        for (let i = 0; i <= nounTokens.length - toks.length; i++) {
            if (toks.every((t, j) => nounTokens[i + j] === t)) {
                nounTokens.splice(i, toks.length);
                break;
            }
        }
    }

    const noun = nounTokens.join(' ').trim();
    if (noun && noun.split(/\s+/).length >= 2) {
        for (const attr of attributes.slice(0, 5)) {
            if (!seoTokens(attr).length) continue;
            pushKeyword(result, `${attr} ${noun}`);
        }
    }

    // One useful combination of up to three explicit attributes, preserving
    // seller order. This avoids nonsense permutations.
    const combo = attributes.slice(0, 3).join(' ').trim();
    if (combo && noun) pushKeyword(result, `${combo} ${noun}`);

    return uniqueStrings(result).slice(0, 20);
}

// ==========================================================
// SEO GENERATOR — FINAL ROBUST VERSION
// ==========================================================
...
async function handleGenerateSEO(req, res) {
    try {
        ...
        // IMPORTANT: productName is the canonical main keyword.
        // The frontend may send productDetails inside mainKeyword/keyword;
        // never treat that combined text as the primary SEO phrase.
        const mainKeyword = productName;
        const deterministicKeywords = buildGuaranteedSEOKeywords(body, productName);
...
2. The primary keyword must be exactly the product name.
3. Create natural search phrases, not word fragments.
4. Never return a standalone attribute such as "inch", "plastic", "black".
5. Every keyword must contain the product or a complete natural noun phrase for the product.
6. Do not duplicate the product phrase by adding/removing random words.
7. Preserve natural attribute order when combining size, material, color, pattern, etc.
8. Do not invent attributes or promotional claims.
9. Do not use online, buy, best, premium, trendy, stylish or similar filler.
10. Return at most 12 useful keywords. Do not add filler to reach a count.

GOOD EXAMPLE:
Product: 8 Inch Plastic Plant Pot
Details: 8 inch, plastic, black, plant pot
Good keywords:
- 8 Inch Plastic Plant Pot
- 8 inch plastic plant pot black
- black plastic plant pot
- 8 inch black plant pot
- 8 inch black plastic plant pot
- black plant pot

BAD EXAMPLES:
- inch
- plastic
- plant pot 8 Inch Plastic Plant Pot
- plastic 8 Inch Plastic Plant Pot
- inch plastic black plant pot pot
...
`;
        ...
        // Filter AI output against the same strict product-relevance rules,
        // then keep deterministic seller-fact keywords first.
        ...
        // Final safety pass: reject fragments and repeated product text.
        keywords = keywords.filter(keyword => {
            ...
            if (productTokens.length >= 2 && !productTokens.some(t => tokens.includes(t))) return false;
            return !/^\d+(?:\.\d+)?$/i.test(value);
        });

        keywords = uniqueStrings([mainKeyword, ...keywords]).slice(0, 20);
