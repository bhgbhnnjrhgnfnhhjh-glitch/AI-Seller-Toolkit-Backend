...
// ==========================================================
// SEO KEYWORD ENGINE — FINAL VERSION 28.0
...
function buildNaturalSEOKeywords(body, productName) {
    ...
    const add = value => {
        ...
        if (!v) return;
        pushKeyword(result, v);
    };

    // Canonical keyword: ALWAYS keep the complete seller product name.
    add(product);

    if (brand && !containsWholePhrase(product, brand)) {
        add(`${brand} ${product}`);
    }

    // Collect seller-supplied facts as complete phrases only.
    const detailText = normalizeSEOKey(body.productDetails);
    const detailPhrases = detailText
        .split(/[,;|\n]+/)
        .map(normalizeSEOKey)
        .filter(Boolean);

    const structured = [
        body.color, body.material, body.fabric, body.size,
        body.pattern, body.design, body.fit, body.occasion,
        body.quantity, body.capacity, body.weight, body.dimensions
    ].map(normalizeSEOKey).filter(Boolean);

    const facts = [];
    const addFact = fact => {
        const value = normalizeSEOKey(fact);
        if (!value || value.length < 2) return;
        if (containsWholePhrase(product, value)) return;
        if (facts.some(x => x.toLowerCase() === value.toLowerCase())) return;
        facts.push(value);
    };

    [...detailPhrases, ...structured].forEach(addFact);

    // Build the noun phrase from the product itself, without letting
    // attributes such as size/material become the noun.
    const words = seoWordList(product);
    const nonNumeric = words.filter(w => !/^\d+(?:\.\d+)?$/.test(w));

    const descriptorWords = new Set([
        'new','pack','set','size','color','colour','with','for','of','and'
    ]);

    const nounCandidates = nonNumeric.filter(w => !descriptorWords.has(w));

    // For common ecommerce names, the final 2 words are the product noun.
    // Example: 8 Inch Plastic Plant Pot -> Plant Pot.
    let nounPhrase = nounCandidates.slice(-2).join(' ');
    if (!nounPhrase) nounPhrase = nonNumeric.slice(-2).join(' ');

    const nounTokens = seoWordList(nounPhrase);
    const nounStart = findSequence(words, nounTokens);

    const colors = new Set([
        'black','white','red','blue','green','yellow','pink','purple','orange',
        'brown','grey','gray','gold','silver','beige','maroon','navy','cream',
        'transparent','multicolor','multicolour'
    ]);
    const materials = new Set([
        'plastic','cotton','silk','wool','linen','metal','steel','stainless steel',
        'wood','wooden','glass','ceramic','leather','rubber','nylon','polyester',
        'acrylic','paper','foam'
    ]);

    const classify = fact => {
        const lower = normalizeSEOKey(fact).toLowerCase();
        const tokens = seoWordList(fact);
        if (colors.has(lower)) return 'color';
        if (materials.has(lower)) return 'material';
        if (tokens.some(t => /^\d+(?:\.\d+)?$/.test(t))) return 'size';
        return 'other';
    };

    // Insert an attribute into the existing product phrase. Never append a
    // new attribute after the product name; that creates unnatural phrases.
    const insertFact = (fact, type) => {
        if (nounStart < 0) return;

        const before = words.slice(0, nounStart);
        const noun = words.slice(nounStart);
        const factWords = seoWordList(fact);
        if (!factWords.length) return;

        if (type === 'color') {
            // Preferred: 8 Inch Black Plastic Plant Pot
            const materialIndex = before.findIndex(w => materials.has(w));
            if (materialIndex >= 0) {
                const copy = [...before];
                copy.splice(materialIndex, 0, ...factWords);
                add([...copy, ...noun].join(' '));
            } else {
                add([...before, ...factWords, ...noun].join(' '));
            }
            // Useful short natural form.
            add(`${fact} ${nounPhrase}`);
            // Prefix form is valid for ecommerce search.
            add(`${fact} ${product}`);
        } else if (type === 'material') {
            // Material before the noun: Cotton Kurti / Plastic Plant Pot.
            add([...before, ...factWords, ...noun].join(' '));
            add(`${fact} ${nounPhrase}`);
        } else {
            add(`${fact} ${nounPhrase}`);
    ...
    // Process each extra fact, but never create a standalone fragment.
    for (const fact of facts.slice(0, 8)) {
        insertFact(fact, classify(fact));
    }

    // Combine color + material when both are explicitly supplied.
    const colorFact = facts.find(f => classify(f) === 'color');
    const materialFact = facts.find(f => classify(f) === 'material');
    if (colorFact && materialFact) {
        add(`${colorFact} ${materialFact} ${nounPhrase}`);
        if (nounStart >= 0) {
            const before = words.slice(0, nounStart);
            add([...before, colorFact, materialFact, ...words.slice(nounStart)].join(' '));
        }
    }

    // Combine multiple useful facts only when the result remains natural.
    if (colorFact && facts.length >= 2) {
        const other = facts.find(f => f !== colorFact && classify(f) !== 'other');
        if (other) add(`${colorFact} ${other} ${nounPhrase}`);
    }

    // Final safety filter.
    const final = [];
    for (const raw of uniqueStrings(result)) {
        const value = normalizeSEOKey(raw);
        if (!value || value.length > 120) continue;
        if (containsBlockedSEOWord(value)) continue;
        if (value.split(/\s+/).length < 2) continue;

        // Never allow a truncated product fragment such as
        // "Inch Plastic Plant Pot". A non-exact keyword must contain the
        // complete noun phrase AND, when it resembles the product, preserve
        // all product words in their original order.
        if (value.toLowerCase() !== product.toLowerCase()) {
            if (nounPhrase && !containsWholePhrase(value, nounPhrase)) continue;

            const valueWords = seoWordList(value);
            const productWords = seoWordList(product);
            const productOrder = productWords.filter(w => valueWords.includes(w));
            const looksLikeProductFragment = productOrder.length >= Math.max(2, productWords.length - 1);
            if (looksLikeProductFragment) {
                const sequence = findSequence(valueWords, productWords);
                if (sequence < 0 && !value.toLowerCase().startsWith(product.toLowerCase())) {
                    // Allow natural inserted-attribute variants only if all
                    // original product words are present.
                    const missing = productWords.some(w => !valueWords.includes(w));
                    if (missing) continue;
                }
            }
        }

        if (final.some(x => x.toLowerCase() === value.toLowerCase())) continue;
        final.push(value);
        if (final.length >= 12) break;
    }

    return final.length ? final : [product];
}
