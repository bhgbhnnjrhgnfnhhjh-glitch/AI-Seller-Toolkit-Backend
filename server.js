...
function buildNaturalSEOKeywords(body, productName) {
    ...
    const details = normalizeSEOKey(body.productDetails);
    const result = [];
    ...
    // 1) Exact seller product name is always the first keyword.
    add(product);

    // 2) Brand + product is safe and natural when a brand was supplied.
    if (brand && !containsWholePhrase(product, brand)) {
        add(`${brand} ${product}`);
    }

    // 3) Collect ONLY seller-provided facts that are not already contained
    //    in the exact product name. Never split a fact into single words.
    const detailPhrases = details
        .split(/[,;|\n]+/)
        .map(normalizeSEOKey)
        .filter(Boolean);
    ...
    const addFact = fact => {
        ...
        if (extraFacts.some(x => x.toLowerCase() === value.toLowerCase())) return;
        extraFacts.push(value);
    };

    for (const phrase of detailPhrases) addFact(phrase);
    ...
    for (const fact of structured) addFact(fact);

    // 4) Find the natural product noun phrase. Example:
    //    "8 Inch Plastic Plant Pot" -> "plant pot".
    const stopDescriptors = new Set([
        "a","an","the","and","with","for","of",
        "new","pack","set","size","color","colour"
    ]);
    const productWords = seoWordList(product);
    const factWords = new Set(extraFacts.flatMap(seoWordList));
    const nounWords = productWords.filter(w =>
        !/^\d+(?:\.\d+)?$/.test(w) &&
        !stopDescriptors.has(w) &&
        !factWords.has(w)
    );

    let nounPhrase = nounWords.slice(-2).join(" ");
    if (!nounPhrase) {
        nounPhrase = productWords
            .filter(w => !/^\d+(?:\.\d+)?$/.test(w))
            .slice(-2)
            .join(" ");
    }

    // 5) Classify common attributes so added facts are inserted naturally
    //    into the product phrase instead of being blindly appended.
    const colorWords = new Set([
        "black","white","red","blue","green","yellow","pink","purple",
        "orange","brown","grey","gray","gold","silver","beige",
        "maroon","navy","cream","transparent","multicolor","multicolour"
    ]);
    const sizeWords = new Set([
        "small","medium","large","xl","xxl","xxxl","xs","free size"
    ]);
    const materialWords = new Set([
        "plastic","cotton","silk","wool","linen","metal","steel",
        "stainless steel","wood","wooden","glass","ceramic","leather",
        "rubber","nylon","polyester","acrylic","paper","foam"
    ]);

    const classifyFact = fact => {
        const words = seoWordList(fact);
        const lower = normalizeSEOKey(fact).toLowerCase();
        if (colorWords.has(lower)) return "color";
        if (sizeWords.has(lower) || words.some(w => /^\d+(?:\.\d+)?$/.test(w))) return "size";
        if (materialWords.has(lower)) return "material";
        return "other";
    };

    // 6) Generate natural variants. The exact product word order is always
    //    preserved. New attributes are inserted before the noun phrase.
    for (const fact of extraFacts.slice(0, 8)) {
        const type = classifyFact(fact);
        const productLower = product.toLowerCase();
        const nounLower = nounPhrase.toLowerCase();
        const nounIndex = productLower.lastIndexOf(nounLower);

        // Strong short keyword: "Black Plant Pot".
        add(`${fact} ${nounPhrase}`);

        if (nounIndex >= 0) {
            const beforeNoun = product.slice(0, nounIndex).trim();
            const afterNoun = product.slice(nounIndex + nounPhrase.length).trim();

            if (type === "color") {
                // Color normally sits immediately before the product material/
                // noun: "8 Inch Black Plastic Plant Pot".
                const wordsBefore = beforeNoun.split(/\s+/).filter(Boolean);
                const materialIndex = wordsBefore.findIndex(w => materialWords.has(w.toLowerCase()));
                if (materialIndex >= 0) {
                    wordsBefore.splice(materialIndex, 0, fact);
                    add(`${wordsBefore.join(" ")} ${nounPhrase}${afterNoun ? ` ${afterNoun}` : ""}`);
                } else {
                    add(`${beforeNoun} ${fact} ${nounPhrase}${afterNoun ? ` ${afterNoun}` : ""}`);
                }

                // Also provide a prefix form when useful: "Black 8 Inch...".
                add(`${fact} ${product}`);
            } else {
                add(`${beforeNoun} ${fact} ${nounPhrase}${afterNoun ? ` ${afterNoun}` : ""}`);
            }
    ...
    // 7) Combine two explicit extra facts without breaking the original
    //    product order. Never output standalone fragments.
    if (extraFacts.length >= 2 && nounPhrase) {
        const first = extraFacts[0];
        const second = extraFacts[1];
        add(`${first} ${second} ${nounPhrase}`);
    }

    // 8) Final hard safety filter. Every non-exact keyword must contain the
    //    natural noun phrase, so fragments like "inch" or "plastic" can
    //    NEVER be returned.
    const final = [];
    for (const raw of uniqueStrings(result)) {
        ...
        if (value.toLowerCase() !== product.toLowerCase() && nounPhrase) {
            if (!containsWholePhrase(value, nounPhrase)) continue;
        }
