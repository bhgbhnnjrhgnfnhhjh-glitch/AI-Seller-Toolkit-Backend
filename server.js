...
function buildInlineSEOKeywords(mainKeyword, productName, brand) {
    ...
    // Example: Blue Cotton Kurti Floral Print.
    if (d1 && d2 && d3) {
        pushKeyword(
            result,
            `${d1} ${product} ${d2} ${d3}`
...
function isValidSEOKeyword(keyword, productName, brand = "") {
    ...
    // Product relevance: at least one meaningful product token must remain.
    const productTokens = normalizeWords(productName);
    if (!productTokens.length) return false;
    if (!productTokens.some(token => tokens.includes(token))) return false;
