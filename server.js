...
function buildGuaranteedSEOKeywords(body, productName) {
    const result = [];
    const brand = cleanSEOKeyword(body.brand);
    const details = cleanSEOKeyword(body.productDetails);
    const main = cleanSEOKeyword(body.mainKeyword || body.keyword);

    pushKeyword(result, main || productName);
    pushKeyword(result, productName);

    // Brand + product only when it is not already present as the same phrase.
    if (brand && !productName.toLowerCase().startsWith(brand.toLowerCase())) {
        pushKeyword(result, `${brand} ${productName}`);
    }

    // Use explicit comma/semicolon-separated seller phrases from details.
    ...
    for (const phrase of phrases) {
        ...
        pushKeyword(result, phrase);
        pushKeyword(result, `${phrase} ${productName}`);
        pushKeyword(result, `${productName} ${phrase}`);
    }

    // Useful combinations from explicit words in details, without inventing facts.
    const detailWords = normalizeWords(details)
        .filter(w => !/^\d+(?:\.\d+)?$/.test(w))
        .filter(w => w !== "capacity");
    ...
    for (const word of detailWords) {
        if (productWords.includes(word)) continue;
        if (["and", "with", "the", "a", "an", "of"].includes(word)) continue;
        candidateWords.push(word);
    }

    const uniqueWords = [...new Set(candidateWords)];
    for (const word of uniqueWords.slice(0, 6)) {
        pushKeyword(result, `${word} ${productName}`);
    }
