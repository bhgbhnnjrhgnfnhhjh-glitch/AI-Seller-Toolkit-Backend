...
function isSameOrContainsBrand(keyword, brand) {
    const k = cleanSEOKeyword(keyword).toLowerCase();
    const b = cleanSEOKeyword(brand).toLowerCase();
    if (!k || !b) return false;
    const tokens = new Set(seoTokens(k));
    const brandTokens = seoTokens(b);
    return brandTokens.length > 0 && brandTokens.every(t => tokens.has(t));
}

function isProductNameOnly(keyword, productName, brand) {
    const k = cleanSEOKeyword(keyword).toLowerCase();
    const base = removeBrandFromProductName(productName, brand).toLowerCase();
...
function buildInlineSEOKeywords(mainKeyword, productName, brand) {
    ...
    // Never treat a full sentence as an SEO keyword.
    // The seller's primary field may contain a description such as
    // "Cotton kurti with floral print. Blue color."; only natural
    // product keywords should be generated from its factual attributes.
...
function buildFactualSEOKeywords(mainKeyword, productName, facts, body = {}) {
    ...
    // 4. Brand is intentionally excluded from SEO combinations.
    // It is already available as a separate structured seller field.
...
const SEO_CONNECTOR_WORDS = new Set([
    "with", "and", "or", "for", "the", "a", "an", "is", "are",
    "was", "were", "of", "in", "on", "at", "by", "to", "from",
    "this", "that", "these", "those", "as", "into", "using",
    "used", "made", "featuring", "features", "available", "color",
    "colour"
]);

function hasSEOConnectorWord(keyword) {
    const tokens = seoTokens(keyword);
    return tokens.some(token => SEO_CONNECTOR_WORDS.has(token));
}
...
function hasUnnaturalBrandPlacement(keyword, brand, productName) {
    ...
    // If a brand is used, it must be a clean prefix of the keyword.
    // Reject suffixes and constructions such as:
    // "floral Test Brand Cotton Kurti" or "Cotton Kurti Test Brand".
    const brandIndex = value.indexOf(b);
    if (brandIndex < 0) return false;

    if (brandIndex > 0) return true;
    if (value.endsWith(` ${b}`)) return true;

    // Brand may occur only once.
    const second = value.indexOf(b, b.length);
    if (second >= 0) return true;

    // If the product name starts with the same brand, keep the same order.
...
function isNaturalSEOKeyword(keyword, productName, mainKeyword = "", brand = "") {
    ...
    if (hasSEOConnectorWord(value)) return false;
...
function filterSEOKeywords(keywords, productName, mainKeyword = "", brand = "") {
    const result = [];
    const baseProduct = removeBrandFromProductName(productName, brand);

    for (const keyword of uniqueStrings(keywords)) {
        const value = cleanSEOKeyword(keyword);
        if (!value) continue;

        // Brand is already a separate seller field. Do not turn the brand
        // into repeated SEO combinations or make it mandatory in keywords.
        if (brand && isSameOrContainsBrand(value, brand)) continue;

        // Never accept the raw product name as a keyword when it is only
        // the brand + product name. The SEO engine should describe the item,
        // not repeatedly echo the brand field.
        if (isProductNameOnly(value, productName, brand)) continue;

        if (!isValidSEOKeyword(value, baseProduct || productName, mainKeyword, "")) continue;

        const normalized = seoTokens(value).join(" ");
        if (!normalized) continue;

        // Avoid trivial variants that differ only by one modifier when the
        // keyword has no new factual attribute.
        if (result.some(existing => seoTokens(existing).join(" ") === normalized)) {
            continue;
        }
...
async function handleGenerateSEO(req, res) {
    try {
        ...
        const seoBrand = cleanSEOKeyword(body.brand);
        const seoProductName = removeBrandFromProductName(productName, seoBrand) || productName;
...
12. Do not include the brand in SEO keyword combinations when the brand is supplied separately; avoid repeated brand + product variants.
...
`;
        ...
        // Never force the raw primary field into the result. It may be a
        // sentence/description rather than a keyword. Always start with a
        // clean product keyword instead.
        keywords = keywords.filter(keyword =>
            keyword.toLowerCase() !== mainKeyword.toLowerCase()
        );

        const cleanProductKeyword = cleanSEOKeyword(seoProductName)
            .replace(/^[\s,.-]+|[\s,.-]+$/g, "");

        if (cleanProductKeyword) {
            keywords = keywords.filter(keyword =>
                keyword.toLowerCase() !== cleanProductKeyword.toLowerCase()
            );
            keywords.unshift(cleanProductKeyword);
        }

        keywords = uniqueStrings(keywords).slice(0, 20);

        if (!keywords.length) {
            keywords = [cleanProductKeyword || productName];
        }
