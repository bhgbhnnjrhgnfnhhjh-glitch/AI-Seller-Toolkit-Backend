...
function removeBrandFromSEOProduct(productName, brand) {
    ...
    const start = findSequence(productTokens, brandTokens);
    if (start >= 0) {
        productTokens.splice(start, brandTokens.length);
        product = productTokens.join(" ");
    }

    return cleanSEOKeyword(product);
}

function hasFullBrand(keyword, brand) {
    const k = normalizeWords(keyword);
    const b = normalizeWords(brand);
    if (!b.length) return false;
    return b.every(token => k.includes(token));
}

function buildInlineSEOKeywords(mainKeyword, productName, brand) {
    const result = [];
    const source = cleanSEOKeyword(mainKeyword);
    const product = removeBrandFromSEOProduct(productName, brand);
    if (!source || !product) return result;

    const productTokens = normalizeWords(product);
    const head = productTokens[productTokens.length - 1] || "";
    if (!head) return result;

    const inline = extractInlineSEOAttributes(source, product, brand);
    const raw = inline.descriptors.map(cleanSEOKeyword).filter(Boolean);

    // Remove connector/filler words and build multi-word factual attributes.
    const stop = new Set(["with","and","or","for","the","a","an","is","are","of","to","in","on","at","by","from","color","colour"]);
    const tokens = raw.flatMap(v => normalizeWords(v)).filter(t => !stop.has(t));
    const cleanTokens = uniqueStrings(tokens);

    // Detect common factual attribute groups from the seller's exact words.
    const has = t => cleanTokens.includes(t);
    const colors = ["blue","black","white","red","green","yellow","pink","purple","orange","brown","grey","gray","beige","maroon","navy","gold","silver"].filter(has);
    const patterns = [];
    if (has("floral") && has("print")) patterns.push("floral print");
    if (has("striped") && has("print")) patterns.push("striped print");
    if (has("printed") && has("print")) patterns.push("printed print");
    if (!patterns.length && has("floral")) patterns.push("floral");

    // The clean base product is always useful.
    pushKeyword(result, product);

    // Pattern + product head (e.g. Floral Print Kurti).
    for (const pattern of patterns) {
        pushKeyword(result, `${pattern} ${head}`);
    }

    // Color + full product (e.g. Blue Cotton Kurti).
    for (const color of colors) {
        pushKeyword(result, `${color} ${product}`);
    }

    // Color + pattern + product head (e.g. Blue Floral Print Kurti).
    for (const color of colors) {
        for (const pattern of patterns) {
            pushKeyword(result, `${color} ${pattern} ${head}`);
            pushKeyword(result, `${color} ${product} ${pattern}`);
        }
    }

    // Full factual combination when multiple attributes are explicit.
    if (colors.length && patterns.length) {
        for (const color of colors) {
            for (const pattern of patterns) {
                pushKeyword(result, `${color} ${pattern} ${product}`);
            }
        }
    }
...
const SEO_BLOCKED_WORDS = new Set([
    ...
    "guaranteed","guarantee","original","top","number one","no 1","no.1"
]);

const SEO_CONNECTOR_WORDS = new Set([
    "with","and","or","for","the","a","an","is","are","of","to","in","on","at","by","from","color","colour"
]);

function containsBlockedSEOWord(keyword) {
    const lower = keyword.toLowerCase();
    for (const blocked of SEO_BLOCKED_WORDS) {
        if (lower === blocked || lower.includes(` ${blocked} `) || lower.startsWith(`${blocked} `) || lower.endsWith(` ${blocked}`)) return true;
    }
    return false;
}
...
function hasConnectorWord(keyword) {
    return normalizeWords(keyword).some(token => SEO_CONNECTOR_WORDS.has(token));
}

function hasDuplicateToken(keyword) {
...
function isValidSEOKeyword(keyword, productName, mainKeyword = "", brand = "") {
    ...
    if (hasConnectorWord(value)) return false;
    if (hasDuplicateToken(value)) return false;
    if (/[.,;:!?]/.test(value)) return false;
    if (brand && hasFullBrand(value, brand)) return false;
...
async function handleGenerateSEO(req, res) {
    try {
        ...
        const brand = cleanSEOKeyword(body.brand);
        const seoProductName = removeBrandFromSEOProduct(productName, brand);
...
`;
        ...
        // If AI gives weak/broad variants, keep only deterministic factual keywords.
        if (!keywords.length) {
            keywords = filterSEOKeywords(
                deterministicKeywords,
                seoProductName,
                mainKeyword,
                brand
        ...
        if (!keywords.length) keywords = [seoProductName];
